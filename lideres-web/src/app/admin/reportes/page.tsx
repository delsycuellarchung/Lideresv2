...
export default function ReportesPage() {
  ...
  const selectedRow = React.useMemo(() => {
    ...
    return datos.find(d => String(d.codigo) === String(selectedCodigo) || String(d.token) === String(selectedCodigo));
  }, [datos, selectedCodigo]);
  ...
  const radarEstilosOptions = React.useMemo(() => {
    ...
    // wrap labels by inserting newlines at spaces so they fit the radar without shrinking
    const wrappedLabels = labels.map((l:any) => String(l).split(/\s+/).join('\n'));
    let source: any[] = [];
    if (selectedCodigo) {
    ...
    const displayLabels = wrappedLabels.map((l: any) => String(l));
    return {
      series,
      options: {
        ...
        grid: { show: false },
        xaxis: { categories: displayLabels, labels: { formatter: (val:any) => String(val), style: { fontSize: '12px', fontFamily: 'inherit', fontWeight: '400', colors: displayLabels.map(() => '#000000') } } },
  ...
  const handleDownloadPdf = async () => {
    try {
      ...
      // If a specific evaluado is selected, capture only the visible report snapshot
      let targetElement: HTMLElement | null = null;
      let fileBaseName: string | null = null;
      if (selectedCodigo) {
        // clone the rendered report so charts/tables remain as shown
        const clone = el.cloneNode(true) as HTMLElement;
        // try to get a presentable display name for the selected evaluado
        const sel = datos.find(dd => String(dd.codigo) === String(selectedCodigo));
        const displayName = sel ? String(sel.nombre || sel.codigo || '').toUpperCase() : String(selectedCodigo).toUpperCase();
        if (displayName) fileBaseName = `reporte-${displayName}`;
        // attempt to replace the entire search area (label + input) with the evaluado name
        try {
          const searchInput = clone.querySelector('input[aria-label="Buscar evaluado"]');
          if (searchInput) {
      ...
      pdf.save(fileName);
      // cleanup temporary element if used
      if (targetElement) {
        try { document.body.removeChild(targetElement); } catch (err) { /* ignore */ }
      }
    } catch (e) {
...
const radarOptions = React.useMemo(() => {
    // Use full competency names so labels render completely on the radar chart
    const labels = [
      'Comunicación y dirección',
      'Respeto y confianza',
      'Desarrollo de equipo y Empowerment',
      'Adaptabilidad y resiliencia',
      'Motivación e influencia'
    ];
    // wrap competency labels to multiple lines to preserve chart size
    const displayLabels = labels.map(l => String(l).split(/\s+/).join('\n'));
  ...
  return (
    <section ref={reportRef} style={{ padding: 24 }}>
      ...
      <div style={{ marginTop: -40, marginBottom: 14, maxWidth: 720 }}>
        <label style={{ display: 'block', margin: '0 0 8px 0', fontWeight: 600 }}>Buscar evaluado por código o nombre</label>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div style={{ fontWeight: 800, fontSize: 20, color: '#0f172a', minWidth: 160, marginLeft: 'auto', textAlign: 'right' }}>
            {selectedRow ? `Evaluadores: ${selectedRow.evaluadores ?? '-'}` : ''}
          </div>
        </div>
      </div>
  ...
  : [];
                ...
                const compsRaw = [
                  { key: 'comunicacion', label: 'COMUNICACIÓN Y DIRECCIÓN' },
                  { key: 'respeto', label: 'RESPETO Y CONFIANZA' },
                  { key: 'desarrollo', label: 'DESARROLLO DE EQUIPO Y EMPOWERMENT' },
                  { key: 'adaptabilidad', label: 'ADAPTABILIDAD Y RESILIENCIA' },
                  { key: 'motivacion', label: 'MOTIVACION E INFLUENCIA' }
                ];
                const compsOrdered = compsRaw
                  .map(c => ({ ...c, val: typeof d[c.key] === 'number' ? d[c.key] : Number(d[c.key]) }))
                  .filter(c => typeof c.val === 'number' && !isNaN(c.val))
                  .sort((a, b) => b.val - a.val);
                // If for some reason none are numeric, fall back to original order
                const headerComps = compsOrdered.length ? compsOrdered : compsRaw.map(c => ({ ...c, val: d[c.key] ?? null }));
                return (
                  <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: 16, fontSize: 12 }}>
                    <thead>
                      ...
                      <tr style={{ background: '#e8f4fb', fontWeight: 700 }}>
                        {headerComps.map(h => (
                          <td key={h.key} style={{ padding: '6px 8px', border: '1px solid #ccc', textAlign: 'center' }}>{h.label}</td>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      <tr>
                        {headerComps.map(h => (
                          <td key={h.key} style={{ padding: '6px 8px', border: '1px solid #ccc', textAlign: 'center' }}>{(d[h.key] ?? '-') as any}</td>
                        ))}
                      </tr>
                    </tbody>
                  </table>
                  ...
                  {typeof window !== 'undefined' && ReactApexChart ? (
                    selectedCodigo ? (
                      (radarOptions.series && radarOptions.series.length) ? (
                        <ReactApexChart options={radarOptions.options} series={radarOptions.series} type="radar" height={700} width={700} />
                      ) : (
                        <ReactApexChart options={placeholderRadarOptions.options} series={placeholderRadarOptions.series} type="radar" height={700} width={700} />
                      )
                    ) : (
                      <ReactApexChart options={placeholderRadarOptions.options} series={placeholderRadarOptions.series} type="radar" height={700} width={700} />
                    )
            ...
            <div data-pdf-page>
              ...
              {(() => {
                ...
                if (!source.length) return null;
                const d = source[0];
                let estilosKeys = Object.keys(d.estilos || {});
                if (!estilosKeys.length) return null;
                // order estilos by value descending for this evaluado
                estilosKeys = estilosKeys
                  .map(k => ({ k, v: typeof d.estilos?.[k] === 'number' ? d.estilos[k] : Number(d.estilos?.[k]) }))
                  ...
                  {typeof window !== 'undefined' && ReactApexChart ? (
                    selectedCodigo ? (
                      (radarEstilosOptions.series && radarEstilosOptions.series.length) ? (
                        <ReactApexChart options={radarEstilosOptions.options} series={radarEstilosOptions.series} type="radar" height={700} width={700} />
                      ) : (
                        <ReactApexChart options={placeholderRadarOptions.options} series={placeholderRadarOptions.series} type="radar" height={700} width={700} />
                      )
                    ) : (
