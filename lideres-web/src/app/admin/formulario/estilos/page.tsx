...
export default function EstilosPage() {
    ...
    (async () => { try {
      ...
      if (res.ok) {
        ...
        if (Array.isArray(json.estilos) && json.estilos.length > 0) {
          setItems(json.estilos.map((it: any) => typeof it === 'string' ? it : String(it.nombre ?? it)));
          return;
        }
      }
      ...
      if (raw) {
        ...
        setItems(parsed.map((it: any) => typeof it === 'string' ? it : String(it.nombre ?? it)));
      }
  ...
  const persist = (next: string[]) => {
    ...
    try { saveFormulario({ estilos: next }); } catch (e) { console.warn(e); }
  };
  ...
  return (
    <div style={{ padding: 28 }}>
      <h2 style={{ margin: "-90px 0 16px 0", fontSize: 28, fontWeight: 800 }}>ESTILOS</h2>
      ...
      {open && (
        <DraggableModal id="modal-estilos" isOpen={open} onClose={() => { setOpen(false); setEditIndex(null); setName(""); }} minWidth={520} minHeight={160} overlayClassName="modal-overlay" modalClassName="modal" centerOnOpen={true}>
